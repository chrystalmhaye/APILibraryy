...
function generateAccessToken() {
    $key = 'server_hack';
    $iat = time();
    $accessExp = $iat + 60; // 1 minute expiration
    $payload = [
...
function validateToken($request, $response, $next) {
    ...
    try {
        ...
        if (!$tokenData || $tokenData['used'] == 1) {
            return $response->withStatus(401)->write(json_encode(["status" => "fail", "token" => null, "message" => "Sorry, Token is Already Used."]));
        }

    } catch (Exception $e) {
        return $response->withStatus(401)->write(json_encode(["status" => "fail", "token" => null, "message" => "Unauthorized Access"]));
    }
...
$app->get('/authors/get', function (Request $request, Response $response) {
    ...
    // Respond with a new access token
    $response = respondWithNewAccessToken($response);
    $response->getBody()->write(json_encode(["status" => "success", "token" => $response->getHeader('New-Access-Token')[0], "data" => $authors]));
    return $response;
})->add('validateToken');
...
$app->get('/books/get', function (Request $request, Response $response) {
    ...
    // Respond with a new access token
    $response = respondWithNewAccessToken($response);
    $response->getBody()->write(json_encode(["status" => "success", "token" => $response->getHeader('New-Access-Token')[0], "data" => $books]));
    return $response;
})->add('validateToken');
...
$app->post('/books/get_by_author', function (Request $request, Response $response) {
    ...
    if (empty($author_id)) {
        return $response->withStatus(400)->write(json_encode(["status" => "fail", "token" => null, "message" => "author_id is required"]));
    }
...
$app->get('/books_authors/get', function (Request $request, Response $response) {
    ...
    // Respond with a new access token
    $response = respondWithNewAccessToken($response);
    $response->getBody()->write(json_encode(["status" => "success", "token" => $response->getHeader('New-Access-Token')[0], "data" => $relations]));
    return $response;
})->add('validateToken');
